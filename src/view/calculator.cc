...
Calculator::Calculator(QWidget *parent)
  ...
  connect(ui->pushButton_brck_op, SIGNAL(clicked()), this,
          SLOT(digit_nambers()));
  connect(ui->pushButton_brck_cl, SIGNAL(clicked()), this,
          SLOT(digit_nambers()));
...
void Calculator::on_pushButton_clear_clicked() {
  if (!ui->lineEdit->text().isEmpty()) {
    ...
    if (((value.data()[value.size() - 1]) == '(') &&
        ((value.data()[value.size() - 2]).isLetter())) {
...
void Calculator::on_pushButton_graph_clicked() {
  if (!ui->lineEdit->text().isEmpty()) {
    ...
    s21::DataPlot data_plot_input(ui->lineEdit->text().toStdString(), xBegin,
                                  xEnd);
    std::pair<QVector<double>, QVector<double>> data_plot;

    ui->widget->xAxis->setRange(data_plot_input.x_begin_,
                                data_plot_input.x_end_);
    ui->widget->yAxis->setRange(data_plot_input.x_begin_,
                                data_plot_input.x_end_);
    //     ui->widget->yAxis->setRange(ui->lineEdit_y_min->text().toDouble(),
    //     ui->lineEdit_y_max->text().toDouble());
    try {
      data_plot = controller.PlotCalculation(data_plot_input);
      ui->widget->addGraph();
      //            ui->widget->graph(0)->setData(data_plot.first,
      //            data_plot.second);
