...
void Credit::on_calculate_clicked() {
  ...
  ui->lineEdit_total_sum->setText(
      QString::number(output_final_payment, 'g', 20));
  ui->lineEdit_overpayment->setText(
      QString::number(output_overpayment_loan, 'g', 20));

  for (int i = 1; i <= period_in_months; i++) {
    ui->listWidget_pay->addItem(
        QString::number(i, 'g', 20) + " month: " +
        QString::number(output_monthly_payment.at(i - 1), 'g', 20));
  }
}