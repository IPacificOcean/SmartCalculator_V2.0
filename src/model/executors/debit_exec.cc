...
s21::DataDeposit &s21::CalculatorDebit::DebitCalculation(
  ...
  if (capitalization == MONTHLY_CAP) {
    ...
    total_interest = (deposit_sum * percent_rate * deposit_period *
...
Дв — итоговый доход, то есть размер вклада на конец срока включая сумму открытия
и начисленный процент, P — начальный депозит, N — годовая ставка, разделенная на
100, T — срок договора в месяцах.
...
Выяснив годовую ставку, периоды и виды начисления процентов, можно посчитать
доход по вкладу. Простые проценты начисляются по следующей формуле: S =
(P*I*T/K)/100
...
Если вкладчик открыл депозит на 350 000 руб. сроком на 9 месяцев под 4,7%,
процентный доход по вкладу составит:

(350000*4.7*273/365)/100 = 12303,7 (сумма может отличаться в зависимости от
