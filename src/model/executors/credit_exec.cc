...
s21::DataCredit &s21::CalculatorCredit::CreditCalculation(
  ...
  if (l_type == ANNUITY) {
    ...
    for (int i = 0; i < period_in_months; ++i) {
      common_pay_per_months = pay_per_months + (tmp * percent_rate / 100 *
                                                AVERAGE_DAYS_IN_MONTH / 365);
...
Дифференцированный платеж — это система погашения кредита, при которой заемщик
ежемесячно вносит разные суммы, размер которых с каждым разом уменьшается.

Пример расчета: сумма кредита — 300 000 рублей, срок кредита — 6 месяцев, ставка
по кредиту — 20%. Погашение кредита осуществляется дифференцированными
платежами:

1. Ежемесячный платеж по основному долгу = сумма кредита / количество платежных
периодов в течение всего срока кредита.

300 000 рублей / 6 месяцев = 50 000 рублей.

2. Ежемесячная сумма начисленных процентов по кредиту = остаток основного долга
в текущем периоде * годовая процентная ставка * число дней в платежном периоде
(от 28 до 31) / число дней в году (365 или 366).
...
////////////////////////////
Пример расчета аннуитетного платежа (расчеты лучше производить в Microsoft
Excel, также можно воспользоваться функцией ПЛТ).

Условие: сумма кредита — 1 000 000 рублей, срок — три года (36 месяцев), ставка
— 20%. Погашение осуществляется аннуитетными платежами.

1. Ставка по кредиту в месяц = годовая процентная ставка / 12 месяцев 20%/12
месяцев/100 = 0,0166666666666667

2. Коэффициент аннуитета =
(0,0166666666666667*(1+0,0166666666666667)^36/((1+0,0166666666666667)^36—1)=0,0371635833597405

3. Ежемесячный аннуитетный платеж = 0,0371635833597405*1 000 000 рублей = 37 164
рублей

4. Итого переплата по кредиту составила 337 889 рублей

При погашении данного кредита дифференцированными платежами сумма уплаченных
процентов по нему составила бы 308 915 рублей
*/
